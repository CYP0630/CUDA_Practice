...
__global__ void sobelInCuda(unsigned char* dataIn, unsigned char* dataOut, int imgHeight, int imgWidth)
{
    ...
    {
        Gx = dataIn[(yIndex - 1) * imgWidth + xIndex + 1] +  dataIn[yIndex * imgWidth + xIndex + 1] + dataIn[(yIndex + 1) * imgWidth + xIndex + 1]
            - (dataIn[(yIndex - 1) * imgWidth + xIndex - 1] + dataIn[yIndex * imgWidth + xIndex - 1] + dataIn[(yIndex + 1) * imgWidth + xIndex - 1]);
        Gy = dataIn[(yIndex - 1) * imgWidth + xIndex - 1] +  dataIn[(yIndex - 1) * imgWidth + xIndex] + dataIn[(yIndex - 1) * imgWidth + xIndex + 1]
            - (dataIn[(yIndex + 1) * imgWidth + xIndex - 1] + dataIn[(yIndex + 1) * imgWidth + xIndex] + dataIn[(yIndex + 1) * imgWidth + xIndex + 1]);
...
void sobel(Mat srcImg, Mat dstImg, int imgHeight, int imgWidth)
{
    ...
    for (int i = 1; i < imgHeight - 1; i++)
    {
        ...
        {
            Gx = (dataUp[j + 1] + data[j + 1] + dataDown[j + 1]) - (dataUp[j - 1] +  data[j - 1] + dataDown[j - 1]);
            Gy = (dataUp[j - 1] + dataUp[j] + dataUp[j + 1]) - (dataDown[j - 1] + dataDown[j] + dataDown[j + 1]);
...
int main()
{
    Mat grayImg = imread("test01.jpg", 0);
    ...
    imshow("1_sobel", dstImg);

    //save the output image
    imwrite("gpu.jpg", dstImg);
